...
function buildPricePath(open: number, high: number, low: number, close: number, steps: number): number[] {
  const path: number[] = []
...
export async function GET(req: NextRequest) {
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      if (isLive) {
        // ── LIVE PATH: subscribe to tick-bus for new ticks only ───────────────
        // Send raw UTC epoch — the chart component applies TZ offset uniformly.
        const unsub = tickBus.subscribe(pairId, (tick: Tick) => {
          if (aborted) return
          send({
            price: tick.price,
            candle: {
              time:  tick.candle.time,  // raw UTC seconds, no offset
        ...
        // ── SIMULATED PATH: DB candle polling ────────────────────────────────
        ...
        ;(async () => {
          while (!aborted) {
            ...
            let liveHigh = open, liveLow = open
            ...
            while (!aborted && stepIndex < path.length) {
              ...
              send({
                price,
                candle: { time: candleStartSec, open, high: liveHigh, low: liveLow, close: price },
                regime: 'ranging',
              })
